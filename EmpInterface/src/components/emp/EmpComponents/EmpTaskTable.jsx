...
const TaskTable = () => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const { tasks, role } = response.data; // Assuming API sends back role and tasks
      setTasks(tasks);
      setUserRole(role); // Set user role (employee, project manager, admin)
      setError(null);
    } catch (err) {
            ...
            {tasks.map((task, index) => (
              ...
              >
                ...
                <td className="px-6 py-4 font-semibold">
                  {userRole === "Project Manager" ? "N/A" : task.employeeName || "N/A"}
                </td>
                <td
